import StepSelect from "@/components/select/StepSelect";
...
const FilterBox: React.FC<Props> = ({ onChangeFilter }) => {
  ...
  const onJobTitleSearch: SearchProps["onSearch"] = (value) =>
    onChangeFilter("keyword", value);

  const onRecruiterNameSearch: SearchProps["onSearch"] = (value) =>
    onChangeFilter("recruiter", value);

  const onChangeStep = (value: string) => {
    onChangeFilter("counts", value);
  };
  ...
  return (
    <div className="mb-4 flex flex-wrap justify-end gap-4">
      ...
      <Search
        ...
        onSearch={onRecruiterNameSearch}
        enterButton
      />
    </div>
  );
